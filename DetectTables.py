...
def convertPdfsToImages():
    DPI = 72
    root_dir = os.getcwd()
    pdf_source_dir = os.path.join(root_dir, 'data', 'Final_test/','prog_pdfs_small')
    PDFs = [pdf for pdf in os.listdir(pdf_source_dir) if pdf.endswith(".pdf")]
    for i, pdf in enumerate(PDFs):
        print("Converting {}/{}: {}".format(i+1, len(PDFs), pdf))
        pages = convert_from_path(os.path.join(pdf_source_dir, pdf), DPI) # DPI: 500
        for i, page in enumerate(pages):
            page.save(os.path.join( pdf_source_dir, pdf.split(".")[0] + "_Page{}-{}.png".format(i+1, DPI)), 'PNG')
...
def preProcessSampleImages():
    ...
    image_source_dir = os.path.join(root_dir, 'ProgPDFs6000','ProgPDFs6000_72png/')
    data_root = os.path.join(root_dir, 'data')

    image_target_dir = os.path.join(root_dir, 'ProgPDFs6000', 'ProgPDFs6000_72png_processed/')
    ...
    for i, image in enumerate(image_list):
        ...
        img = cv2.imread(os.path.join(image_source_dir, image))
        print(img.shape)
        # img = np.array(skimage.io.imread(os.path.join(image_source_dir, image), plugin='tifffile'))
...
def interpolateImages():
    root_dir = os.getcwd()
    source_dir = os.path.join(root_dir, 'data', 'Final_Test', 'isri-ocr-small-lr')
    dest_dir = os.path.join(root_dir, 'data', 'Final_Test', 'isri-ocr-small-interp')
    ...
    for image in images:
        print(image)
        # Obtained by comparing the 72, 100 DPI images with their corresponding HR versions.
        if image.endswith("-72.jpg"):
            scaling_factor = 4.17
        elif image.endswith("-100.jpg"):
            scaling_factor = 3

        img = cv2.imread(os.path.join(source_dir, image))
        interpolated_image = cv2.resize(img, dsize=None, fx=scaling_factor, fy=scaling_factor, interpolation=cv2.INTER_CUBIC)
        cv2.imwrite(os.path.join(dest_dir, image), interpolated_image)
