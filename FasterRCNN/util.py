import math
import numpy as np
from Parameters import RESIZE_MIN_SIZE, RESIZE_MAX_SIZE, DEFAULT_ANCHOR_SCALES, DEFAULT_ANCHOR_RATIOS
...
def transform_np_inplace(coords, reg_targets):
    ...
    ## Retrieving width and height of BBox
    coords[:, 2] -= coords[:, 0]
    coords[:, 3] -= coords[:, 1]
    ...
    ## Retrieving center of the BBox
    coords[:,0] += coords[:,2]/2
    coords[:,1] += coords[:,3]/2

    ## New central co-ords of the BBox
    coords[:,0] += tx * coords[:,2]
    coords[:,1] += ty * coords[:,3]

    ## New width, height of the BBox
    coords[:,2] *= np.exp(tw)
    coords[:,3] *= np.exp(th)

    ## New x1, y1 of the BBox
    coords[:,0] -= coords[:,2]/2
    coords[:,1] -= coords[:,3]/2

    np.round(coords, out=coords)

    ## Converting width, height into x2, y2
    coords[:,2] += coords[:,0]
    coords[:,3] += coords[:,1]
...
def create_Prediction_Dict(rois, roi_prob):

    pred = {}
    for i in range(rois.shape[0]):
        x1, y1, x2, y2 = rois[i][0], rois[i][1], rois[i][2], rois[i][3]
        prob = roi_prob[i]
        pred["{}".format(i)] = (x1, y1, x2, y2, prob)
